...
function wxParse(bindName = 'wxParseData', type = 'html', data = '<div class="color:red;"></div>', target, imagePadding) {
  ...
  that.bindData = bindData //新增这一行
  that.wxParseImgLoad = wxParseImgLoad;
  that.wxParseImgTap = wxParseImgTap;
  // that.getUserInfo = getUserInfo;
  // that.inputChangeHandle = inputChangeHandle;
  this.jumpMp = jumpMp;
  ...
  // bindData.getUserInfo = getUserInfo
  // bindData.inputChangeHandle = inputChangeHandle
  bindData.jumpMp = jumpMp
  bindData.buttonHandle = buttonHandle
...
// 点击获取getUserInfo
// const getUserInfo = e => {
//   console.warn("getUserInfo wxParser", e, this);
...
function wxParseImgTap(e, bindData) {
  ...
  if (typeof (tagFrom) != 'undefined' && tagFrom.length > 0) {
    wx.previewImage({
      ...
      urls: that.__data__.article.imageUrls // 需要预览的图片http链接列表
    })
  }
}
